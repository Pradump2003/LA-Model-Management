...
exports.searchModels = asyncHandler(async (req, res) => {
  ...
  let filterQuery = {
    ...
    $or: [
      ...
      { lastName: searchRegex },
      { slug: searchRegex },
      { division: searchRegex },           // Added division search
      { categories: searchRegex },         // Added categories search
      { gender: searchRegex },             // Added gender search
      { experience: searchRegex },         // Added experience search
      { ethnicity: searchRegex },
      { "location.based": searchRegex },
      ...
      { "portfolio.runwayShows": searchRegex },
      { "features.specialSkills": searchRegex },
      { keywords: searchRegex },
      { metaTitle: searchRegex },
...
exports.getAllModels = asyncHandler(async (req, res) => {
  ...
  // Letter filter (by first letter of firstName)
  if (letter && typeof letter === "string" && letter.trim()) {
    const normalized = letter.trim().toUpperCase().slice(0, 1);
    filterQuery.firstName = new RegExp(`^${escapeRegex(normalized)}`, "i");
  }
